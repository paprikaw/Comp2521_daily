...
Graph newGraph(int nV)
{
   ...
   assert(g != NULL);
   g->nV = nV;  g->nE = 0;
   List *l = malloc(g->nV * sizeof(List));
...
void  insertE(Graph g, Edge e)
{
   ...
   if (ListSearch(g->list[e.w], e.v) != NULL) return;
   ListInsert(g->list[e.v], e.w);
   ListInsert(g->list[e.w], e.v);
   g->nE++;
   }
...
void  removeE(Graph g, Edge e)
{
   ...
   assert(validV(g,e.v) && validV(g,e.w));
   if (ListSearch(g->list[e.w], e.v) == NULL) return;
   ListDelete(g->list[e.w], e.v);
   ListDelete(g->list[e.v], e.w);
   g->nE--;
}
