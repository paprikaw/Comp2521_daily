...
void BSTreeLevelOrder (BSTree t)
{
	...
	// Create a queue to store the tree node
	// and push the first tree to the queue
	Queue storedQueue = newQueue();
	QueueJoin (storedQueue, t);
	BSTree curr;
	while (QueueIsEmpty(storedQueue) == false) {
		
		curr = QueueLeave(storedQueue);
		printf("%d ", curr->value);
		
		if (curr->left != NULL)
		QueueJoin(storedQueue,curr->left);
		if (curr->right != NULL)
		QueueJoin(storedQueue,curr->right);
...
{	
	...
	// The tree is the leave
	if (t->left == NULL && t->right == NULL) {
		return 1;
	} 
	// Return value for left tree and right tree
	int sumL, sumR;
	sumL = BSTreeNumLeaves(t->left);
	sumR = BSTreeNumLeaves(t->right);
	return sumL + sumR;
