...
// Implemented the function of IntListInsertInOrder
// by Xu Bai(z5292197)
...
{
	// When IntList is empty
	if (L->size == 0) {
		IntListInsert(L, v);
		return;
	}

	// If the v is the smallest number in the list
	if (v < L->first->data) {
		struct IntListNode *storedNode = L->first;
		L->first = newIntListNode(v);
		L->first->next = storedNode;
		L->size++;
		return;
	}

	// If the v is the biggest number in the list
	if (v > L->last->data) {
		L->last->next = newIntListNode(v);
		L->last = L->last->next;
		L->size++;
		return;
	}
	...
	// and the element exists in the list is equal
	// to the input number
	if (v == L->first->data && L->size == 1) {
		L->last->next = newIntListNode(v);
		L->last = L->last->next;
		L->size++;
		return;
	...
	// Loop through the int list to find the appropriate 
	// position 
	struct IntListNode *curr = L->first->next;
	struct IntListNode *before = L->first;
	while(curr != NULL) {

		// When the number is already in the list
		if (v == curr->data || v == before->data) {
			before->next = newIntListNode(v);
			before->next->next = curr;
			L->size++;
			return;
		}
		// When the list have looped to the appropriate
		// position
		if (v < curr->data && v > before->data) {
			before->next = newIntListNode(v);
			before->next->next = curr;
